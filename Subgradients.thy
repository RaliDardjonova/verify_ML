...
proof
  ...
  proof (rule)+
    ...
    have "\<exists> v \<in> A. \<forall> u \<in> A. f (u) \<ge> f(x) + (inner (u-x) v)" using convex_has_subgradient assms 
      using \<open>x \<in> A\<close> by blast
    then have "diff_set f A x \<noteq> {}" unfolding diff_set_def unfolding subgradient_def
      using  \<open>x \<in> A\<close> by blast
    have "\<exists> v \<in> A. \<forall> u \<in> A. f (u) \<ge> f(y) + (inner (u-y) v)" using convex_has_subgradient assms 
