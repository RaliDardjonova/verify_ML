...
lemma 
  ...
  fixes m :: nat
  assumes "m>0"
  assumes f_nn: "\<forall>S\<in> (Samples m D). \<forall>z\<in>{0..<m}. f S z \<ge> 0"
  shows expectation_Pi_pmf_sum: 
 "finite {0..<m} \<Longrightarrow> sum (\<lambda> i.  measure_pmf.expectation (Samples m D) (\<lambda> Sz. f Sz i))
      {0..<m} =  measure_pmf.expectation (Samples m D) 
          (\<lambda> Sz. sum (\<lambda> i. f Sz i) {0..<m})" 
...
proof -
  ...
  let ?M = "(Samples (n+1) D)"
  let ?I = "{0..<n}"
  have "\<forall> S \<in> (set_pmf ?M). (ridge_mine S k) \<in> H" 
  proof 
    fix S
    assume "S \<in> ?M" 
    have "finite {0..<n+1}" by auto 
  ...
  then have f_nn : "\<forall> S \<in> (set_pmf ?M). \<forall> i \<in> ?I. ?f i S \<ge> 0" using l_pos 
  ...
  have "\<forall> i\<in> ?I. \<forall> j \<in> ?I. measure_pmf.expectation ?M (\<lambda> S. ?f i S) =
    ...
    by (smt atLeastLessThan_iff le_less_trans le_neq_implies_less less_add_same_cancel1 less_imp_le_nat zero_le_one zero_neq_one)
...
proof -
  ...
  proof -
    ...
    then show ?thesis 
      using integrable_D add_sample_expectation1 n_pos by auto
  qed
